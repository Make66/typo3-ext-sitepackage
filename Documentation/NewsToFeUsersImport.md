# One-off import: news → fe_users (pid 2111)

Console command: `vendor/bin/typo3 news-to-fe-users`
Source: `packages/sitepackage/Classes/Command/NewsToFeUsersCommand.php`

## Why

The Hochschullehre tenant's "Mitglieder" sysfolder (pid 2111 — the `fe_users`
storage folder documented in `packages/members/Documentation/Setup.md`) had
its ~100 members entered as `georgringer/news` records instead of `fe_users`.
The `taketool/members` extension expects real `fe_users` rows in that folder
(`MemberRepository::findAllInStorage()` queries `fe_users` by `pid`), so the
existing news entries needed a one-time conversion.

## Original prompt

> create a script to convert news to fe_users. from the news in pid 2111
> create fe_users in pid 2111 and map header to firstname + lastname, Teaser
> to institute, media to image.
> fill username and password with dummy email and "Test123!". store the
> prompt and the resulting script in docs.

Follow-up decisions made together while planning the script:
- Dummy username/email: `firstname.lastname@example.com` (transliterated,
  deduplicated on collision), not `member-<news-uid>@example.com`.
- Hidden news → imported as **hidden** (`fe_users.disable = 1`) rather than
  skipped.
- Name split: **last word = last name**, everything before it = first name
  (handles multi-part first names such as "Amir Madany Mamlouk" → first
  "Amir Madany", last "Mamlouk").

## Field mapping

| news (`tx_news_domain_model_news`)         | fe_users                        |
|---------------------------------------------|----------------------------------|
| `title` ("Header" in the backend), split on the last space | `first_name` / `last_name` (+ `name` = both joined) |
| `teaser`                                     | `tx_members_institut`           |
| `fal_media` FAL relation (`sys_file_reference`) | `image` FAL relation (same physical file, new reference row) + `fe_users.image` set to the reference count |
| `hidden`                                     | `disable`                       |
| —                                            | `username` / `email` = `firstname.lastname@<domain>` |
| —                                            | `password` = `Test123!`, hashed via the FE password hashing service |

## Usage

```
# preview only, no writes
ddev exec vendor/bin/typo3 news-to-fe-users --dry-run

# run for real
ddev exec vendor/bin/typo3 news-to-fe-users

# options (all optional, shown with their defaults)
ddev exec vendor/bin/typo3 news-to-fe-users \
  --source-pid=2111 --target-pid=2111 --email-domain=example.com
```

Safe to re-run: a news record is skipped if a `fe_users` row with its
generated *base* username (`firstname.lastname@<domain>`, before any
disambiguation suffix) already exists in `--target-pid`. This was verified
live — a second run after the initial import reported "0 to create, 100
already migrated" and left the database unchanged.

Known limitation: if two source news records produce the same base username
(same first/last name) *within the same run*, the second one gets a numeric
suffix (`...2@...`, `...3@...`). A later re-run only recognizes the
un-suffixed base name as "already migrated" — true duplicate-name collisions
across separate runs are not deduplicated. Not an issue for the 2111 dataset
(no duplicate names present).

## Verification performed

Run against the live `taketools-com` DDEV database on 2026-08-17:

- `--dry-run` previewed all 100 non-deleted news records in pid 2111 (3 of
  them hidden) with correctly split names, generated emails, institutes, and
  image flags.
- Real run created 100 `fe_users` rows in pid 2111, 3 with `disable = 1`
  (matching the 3 hidden news records), all passwords hashed with argon2i.
- 77 new `sys_file_reference` rows (`tablenames='fe_users'`,
  `fieldname='image'`) were created, matching the 77 source news records
  that had a `fal_media` image.
- Re-running the command reported 0 created / 100 skipped, and left the
  `fe_users` and `sys_file_reference` row counts unchanged — confirming
  idempotency.

### Bug found and fixed after the initial run

While wiring up "add image to list view item" in the `taketool/members`
plugin, `Member::getImage()` came back empty for every member even though the
`sys_file_reference` rows existed. Root cause: Extbase's `DataMapper`
(`fetchRelatedEager()`) gates FAL relation fetching on the raw
`fe_users.image` column being non-empty — it does **not** resolve the
relation purely from the `sys_file_reference` rows the way TYPO3 core's own
FAL usage does. The original command only created the `sys_file_reference`
rows and never touched that counter column, so it stayed `NULL`.

Fixed in `createFeUser()`: the insert now also sets `image` to the media
count (empty string when there is none). The 100 records created by the
initial run were backfilled directly:

```sql
UPDATE fe_users fu
INNER JOIN (
  SELECT uid_foreign, COUNT(*) AS cnt
  FROM sys_file_reference
  WHERE tablenames='fe_users' AND fieldname='image'
  GROUP BY uid_foreign
) sfr ON sfr.uid_foreign = fu.uid
SET fu.image = sfr.cnt
WHERE fu.pid = 2111;
```

Verified after the fix: loading all `Member` entities for pid 2111 through
the real `MemberRepository`/`ImageService` stack now resolves 75 image URLs
for the 97 frontend-visible members (77 total minus the 2 hidden members that
had an image) — matching the source data exactly.
